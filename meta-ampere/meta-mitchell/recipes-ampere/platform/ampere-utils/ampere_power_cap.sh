...
function usage() {
    echo "Usage: ampere_power_cap.sh [OPTION] <0/1> [type] [value]"
    echo "SOCKET:"
    ...
    echo "TYPE:"
    echo "        DramPwrLimit: Set power limit for DRAM"
    echo "        PLimit: Set SoC power limit"
    echo "        PMinPowerLimit: Set SoC power min limit"
    echo "        BERTTrigger: Trigger one BERT event. Value 0\1"
    echo "        NMITrigger: Trigger an non-maskable interrupt to the host processor. Value 0\1"
}
...
listPathS0=(
    "/xyz/openbmc_project/sensors/power/S0_DRAM_Pwr_Lmt"
...
listPathS1=(
    "/xyz/openbmc_project/sensors/power/S1_DRAM_Pwr_Lmt"
...
case "$operation" in
    ...
    "-s" | "--set")
        ...
        if [[ "$type" == "BERTTrigger" ]] || [[ "$type" == "NMITrigger" ]]; then
            if [[ "$value" == "0" ]] || [[ "$value" == "1" ]]; then
                set_option "$socket" "$st" "$value"
            else
                echo "Only support set 0|1 for BERTTrigger or NMITrigger"
            fi
        else
            set_option "$socket" "$st" "$value"
        fi
