# Altra OpenBMC Porting Guide

This document aims to guide users to port Mt. Jade OpenBMC to a new Altra-based platform. Guideline for features that are not specific for Ampere Altra-based reference platform is out of scope for this document.
...
- [ampere-platform-mgmt][2]: contain to handle Altra error monitoring.
...
Below kernel drivers are required for Ampere Altra based platforms:

* [BMC SSIF driver](https://lkml.org/lkml/2021/7/13/868), enabled by the `CONFIG_SSIF_IPMI_BMC`
...
* smpro-hwmon, errmon and misc device nodes:
...
- Add `bmc-ssif` driver and enable it.
- Enable `obmc-host-ipmi-hw` with `phosphor-ipmi-ssif` in [board config](https://github.com/ampere-openbmc/openbmc/blob/ampere/meta-ampere/meta-jade/conf/machine/mtjade.conf) file:
```
PREFERRED_PROVIDER_virtual/obmc-host-ipmi-hw = "phosphor-ipmi-ssif"
```
- Enable SSIF channel configuration in [channel_config.json](https://github.com/ampere-openbmc/openbmc/blob/ampere/meta-ampere/meta-jade/recipes-phosphor/ipmi/phosphor-ipmi-config/channel_config.json) file.
